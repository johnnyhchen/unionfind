...
#for MESHPIECE_SIZE in 64; do
  for dim in 8192; do
    ...
    aprun -n 2 -N 2 /u/sciteam/sheth/unionFindFP/examples/prob_mesh/mesh $dim $MESHPIECE_SIZE $prob ++ppn 15 +pemap 1-15,17-31 +commap 0,1 > out1
    cp out1 out_1_node

    tree_time=`grep -i "Tree construction time" out1 | cut -d ":" -f 2 | tr -d " "`
    component_time=`grep -i "Components detection time" out1 | cut -d ":" -f 2 | tr -d " "`
    total_time=`grep -i "Final runtime" out1 | cut -d ":" -f 2 | tr -d " "`
    echo "$dim $prob $tree_time $component_time $total_time" >> $logfile
    #exit #comment out1 this line after testing script on interactive node
  done
done
